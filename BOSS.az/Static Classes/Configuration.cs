...
namespace BOSS.az.Static_Classes
{
    ...
    {
        public static void PrintMenu(in string[] menu,int selected)
        {
            for (int i = 0; i < menu.Length; i++)
            {
                if (i == selected && menu[selected] == "Exit")
                {
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine($"{i + 1}. {menu[i]}");
                    Console.ResetColor();
                }
                else if (i == selected && menu[selected] == "Back")
                {
        ...
        {
            return new string[] { "Employer", "Worker", "Exit" };
        }
